...
function fetchTopStocks(){
    fetch(
      "https://www.alphavantage.co/query?function=SECTOR&apikey=FR1IAHH5I843LFTD"
    )
...
// Function to fetch and display the stock data for the searched symbol
...
function fetchStockData(symbol){
    ...
    fetch(`https://www.alphavantage.co/query?function
    =GLOBAL_QUOTE&symbol=${symbol}&apikey=FR1IAHH5I843LFTD`).then(response =>response.json())
    .then(data => {
        ...
        if(quote && quote['10. change percent']){
            ...
            replace('%', '');
            const changeColor =
              parseFloat(changePercent) >= 0 ? "green" : "red";
            const html = `
             <li>
                <span class="symbol">${symbol}</span>
